...
const NAV = [
  ...
  { label: "Inventory", path: "/inventory", icon: Package, perm: "can_view_inventory" },
  { label: "Orders", path: "/orders", icon: ShoppingCart, perm: "can_view_orders" },
  { label: "Reports", path: "/reports", icon: FileText, perm: "can_submit_reports" },
  { label: "Settings", path: "/settings", icon: SettingsIcon, perm: null },
];
...
export default function Layout() {
  ...
  const fetchUnread = async () => {
    try {
      const { data } = await api.get("/api/app/notifications/unread-count");
  ...
  const visibleStoreNav = NAV.filter((n) => !n.perm || isAdmin || hasPermission(n.perm));
  const visibleAdminNav = ADMIN_NAV;
  const current = [...visibleStoreNav, ...visibleAdminNav].find((n) => active(n.path));
    ...
    <>
      <div className="px-5 pt-5 pb-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            <p className="text-base font-black text-slate-900 leading-none tracking-tight">Tetiano</p>
        ...
        <div className="mt-4 flex items-center gap-2 px-3 py-2 bg-emerald-50 border border-emerald-100 rounded-xl">
          ...
          <span className="text-[11px] font-bold text-emerald-700">System is healthy</span>
        </div>
      </div>

      <nav className="flex-1 px-3 overflow-y-auto scroll-thin space-y-4">
        <div>
          <p className="px-3 mb-2 section-label">Navigation</p>
          <div className="space-y-0.5">
            {visibleStoreNav.map((n) => (
        ...
        {isAdmin && (
          <div>
            <p className="px-3 mb-2 section-label" style={{ color: "#8b5cf6" }}>
              Admin Panel
            </p>
            <div className="space-y-0.5">
  ...
  return (
    <div className="min-h-screen flex bg-slate-50" dir="rtl">
      <aside className="hidden lg:flex flex-col fixed h-full sidebar z-20" style={{ width: "var(--sidebar-w)" }}>
        <SidebarInner />
      </aside>
      ...
      <div className="flex-1 flex flex-col min-h-screen" style={{ marginRight: "calc(var(--sidebar-w))" }}>
          ...
          <div className="h-full px-4 sm:px-6 flex items-center justify-between gap-4">
            <div className="flex items-center gap-3 lg:hidden">
              ...
              <div className="flex items-center gap-2">
                ...
                <span className="text-base font-black text-slate-900">Tetiano</span>
              </div>
            </div>
                ...
                {userMenuOpen && (
                  <div className="absolute left-0 top-full mt-2 w-52 bg-white border border-slate-200 rounded-2xl shadow-card-md overflow-hidden anim-scale-in z-50">
                    <div className="px-4 py-3 border-b border-slate-100">
                      <p className="text-xs font-black text-slate-800 truncate">{profile?.full_name || "User"}</p>
                    ...
                    <div className="p-1.5">
                      <Link
                        to="/settings"
                        className="flex items-center gap-2 px-3 py-2 rounded-xl text-xs font-bold text-slate-600 hover:bg-slate-50 transition-colors"
                      ...
                      {isAdmin && (
                        <Link
                          to="/admin/dashboard"
                          className="flex items-center gap-2 px-3 py-2 rounded-xl text-xs font-bold text-brand-600 hover:bg-brand-50 transition-colors"
                          onClick={() => setUserMenu(false)}
                        >
        ...
        <footer className="px-6 py-3 border-t border-slate-100 flex items-center justify-between">
          <p className="text-[11px] text-slate-400 font-medium">(c) 2026 Tetiano - Inventory & CRM System</p>
