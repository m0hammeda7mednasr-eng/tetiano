...
function normalizeShopDomain(raw: string): string {
  ...
  if (!normalized.endsWith(".myshopify.com")) return `${normalized}.myshopify.com`;
  return normalized;
}
...
async function resolveStoreDisplayName(storeId: string): Promise<string> {
  const store = await supabase.from("stores").select("name").eq("id", storeId).maybeSingle();
...
async function upsertLegacyBrandForStore(params: {
  ...
  const existing = await supabase.from("brands").select("*").eq("id", storeId).maybeSingle();
  if (existing.error) {
  ...
  if (existing.data) {
    ...
    if (Object.prototype.hasOwnProperty.call(existing.data, "api_key")) updatePayload.api_key = apiKey;
    if (Object.prototype.hasOwnProperty.call(existing.data, "shopify_api_key")) updatePayload.shopify_api_key = apiKey;
    if (Object.prototype.hasOwnProperty.call(existing.data, "api_secret")) updatePayload.api_secret = apiSecret;

    const update = await supabase.from("brands").update(updatePayload).eq("id", storeId);
...
async function clearLegacyBrandConnection(storeId: string): Promise<void> {
  ...
  const updates: Record<string, unknown> = { updated_at: nowIso(), connected_at: null };
  if (Object.prototype.hasOwnProperty.call(existing.data, "shopify_access_token")) updates.shopify_access_token = null;
  if (Object.prototype.hasOwnProperty.call(existing.data, "access_token")) updates.access_token = null;
  if (Object.prototype.hasOwnProperty.call(existing.data, "shopify_scopes")) updates.shopify_scopes = null;
  if (Object.prototype.hasOwnProperty.call(existing.data, "shopify_location_id")) updates.shopify_location_id = null;
  if (Object.prototype.hasOwnProperty.call(existing.data, "is_configured")) updates.is_configured = false;
  if (Object.prototype.hasOwnProperty.call(existing.data, "is_active")) updates.is_active = false;

  const update = await supabase.from("brands").update(updates).eq("id", storeId);
  if (update.error && !isSchemaCompatibilityError(update.error)) {
    throw update.error;
  }
}
...
async function scopedSelect(
  ...
  select: string,
  storeId: string,
  opts?: { count?: "exact"; single?: boolean; maybeSingle?: boolean; orderBy?: string; orderAsc?: boolean; limit?: number },
) {
  let query: any = supabase.from(table).select(select, opts?.count ? { count: opts.count } : undefined);
  query = query.eq("store_id", storeId);
  if (opts?.orderBy) query = query.order(opts.orderBy, { ascending: opts.orderAsc ?? false });
  ...
  let fallback: any = supabase.from(table).select(select, opts?.count ? { count: opts.count } : undefined);
  fallback = fallback.eq("brand_id", storeId);
  if (opts?.orderBy) fallback = fallback.order(opts.orderBy, { ascending: opts.orderAsc ?? false });
...
async function countScoped(table: string, storeId: string): Promise<number> {
  const result = await scopedSelect(table, "id", storeId, { count: "exact", limit: 1 });
...
router.get("/me", async (req: AuthRequest, res: Response) => {
  ...
  try {
    ...
    return res.status(500).json({ error: "Failed to load current user context" });
  }
});
...
router.get("/products", requireStorePermission("can_view_inventory"), async (req: AuthRequest, res: Response) => {
  const storeId = ensureStoreId(req, res);
  if (!storeId) return;
  ...
  try {
    ...
    if (search) query = query.or(`sku.ilike.%${search}%,title.ilike.%${search}%,barcode.ilike.%${search}%`);
    ...
    if (error && isSchemaCompatibilityError(error)) {
      let fallback: any = supabase
        ...
        .eq("brand_id", storeId)
        .order("updated_at", { ascending: false });
      if (search) fallback = fallback.or(`sku.ilike.%${search}%,title.ilike.%${search}%,barcode.ilike.%${search}%`);
...
router.patch("/products/:id", requireStorePermission("products.manage"), async (req: AuthRequest, res: Response) => {
  const storeId = ensureStoreId(req, res);
  if (!storeId) return;
  ...
  try {
    ...
    if (!before.data) return res.status(404).json({ error: "Product not found" });
...
router.patch(
  ...
  async (req: AuthRequest, res: Response) => {
    ...
    try {
      ...
      if (!variant.data) return res.status(404).json({ error: "Variant not found" });

      const rowStoreId = String((variant.data as any).store_id || (variant.data as any).brand_id || "");
      if (!assertStoreScope(req, rowStoreId)) {
        return res.status(403).json({ error: "Cross-store access is forbidden" });
      }

      await inventoryService.adjustStock(variantId, delta, reason, req.user!.id);
      return res.json({ success: true });
    } catch (error: any) {
      logger.error("App variant stock update failed", { error: error?.message, variantId });
...
router.get(
  ...
  async (req: AuthRequest, res: Response) => {
    ...
    try {
      ...
      const variantStoreId = String((variant.data as any).store_id || (variant.data as any).brand_id || "");
      if (!assertStoreScope(req, variantStoreId)) {
        return res.status(403).json({ error: "Cross-store access is forbidden" });
      }

      const movements = await inventoryService.getStockMovements(variantId, limit);
      return res.json({ movements: movements || [] });
    } catch (error: any) {
      logger.error("App variant movements failed", { error: error?.message, variantId });
      return res.status(500).json({ error: "Failed to load stock movements" });
    }
  },
);

router.get("/orders", requireStorePermission("can_view_orders"), async (req: AuthRequest, res: Response) => {
  const storeId = ensureStoreId(req, res);
  if (!storeId) return;
...
router.get("/orders/:id", requireStorePermission("can_view_orders"), async (req: AuthRequest, res: Response) => {
  const storeId = ensureStoreId(req, res);
  if (!storeId) return;
  ...
  try {
    ...
    if (!orderResult.data) return res.status(404).json({ error: "Order not found" });
    ...
    logger.error("App order details failed", { error: error?.message, orderId });
    return res.status(500).json({ error: "Failed to load order details" });
  }
});

router.get("/customers", requireStorePermission("can_view_orders"), async (req: AuthRequest, res: Response) => {
  const storeId = ensureStoreId(req, res);
  if (!storeId) return;
...
router.get("/reports", requireStorePermission("reports.view"), async (req: AuthRequest, res: Response) => {
  const storeId = ensureStoreId(req, res);
  if (!storeId) return;
    ...
    try {
      ...
      if (report.error && isSchemaCompatibilityError(report.error)) {
        report = await supabase
          .from("daily_reports")
          .select("id, report_date, done_today, blockers, plan_tomorrow, submitted_at, created_at, updated_at")
          .eq("brand_id", storeId)
...
router.post("/reports", requireStorePermission("reports.create"), async (req: AuthRequest, res: Response) => {
  const storeId = ensureStoreId(req, res);
  if (!storeId) return;
  ...
  if (!bodyText) {
    return res.status(400).json({ error: "body_text or done_today is required" });
  }

  const reportDate = String(req.body?.report_date || new Date().toISOString().slice(0, 10));
...
router.post(
  ...
  async (req: AuthRequest, res: Response) => {
    ...
    if (!["image", "audio", "file"].includes(fileType)) {
      return res.status(400).json({ error: "file_type must be image|audio|file" });
    }
    ...
    try {
      ...
      if (report.error) {
        if (isSchemaCompatibilityError(report.error)) {
          return res.status(503).json({
            error: "Report attachments require migration 017_store_per_tenant_v2.sql",
          });
        }
        throw report.error;
      }
      ...
      const bucket = (process.env.REPORTS_STORAGE_BUCKET || "reports-media").trim();
      const storageKey = `${storeId}/${reportId}/${Date.now()}-${crypto.randomBytes(6).toString("hex")}-${fileName}`;

      const signed = await supabase.storage.from(bucket).createSignedUploadUrl(storageKey);
      ...
      logger.error("App report attachment presign failed", { error: error?.message, reportId });
      return res.status(500).json({ error: "Failed to generate upload URL" });
    }
  },
);

router.post("/reports/:id/comments", requireStorePermission("reports.view"), async (req: AuthRequest, res: Response) => {
  const storeId = ensureStoreId(req, res);
  if (!storeId) return;
  ...
  try {
    ...
    logger.error("App report comment failed", { error: error?.message, reportId });
    return res.status(500).json({ error: "Failed to create comment" });
  }
});

router.get("/users", requireStoreRole("admin"), async (req: AuthRequest, res: Response) => {
  const storeId = ensureStoreId(req, res);
  if (!storeId) return;
  ...
  try {
    ...
    const ids = (memberships.data || []).map((row: any) => String(row.user_id));
    ...
    if (ids.length > 0) {
      ...
      profilesMap = new Map((profiles.data || []).map((row: any) => [String(row.id), row]));
    }
    ...
    const users = (memberships.data || []).map((membership: any) => ({
      ...
      full_name: profilesMap.get(String(membership.user_id))?.full_name || null,
      is_active: profilesMap.get(String(membership.user_id))?.is_active ?? true,
    }));
...
router.post("/users", requireStoreRole("admin"), async (req: AuthRequest, res: Response) => {
  const storeId = ensureStoreId(req, res);
  if (!storeId) return;
  ...
  try {
    ...
    return res.status(201).json({ success: true, user_id: userId, membership: membership.data });
  } catch (error: any) {
...
router.patch("/users/:id/role", requireStoreRole("admin"), async (req: AuthRequest, res: Response) => {
  const storeId = ensureStoreId(req, res);
  if (!storeId) return;
  ...
  try {
    const before = await supabase
      ...
      .maybeSingle();
    if (before.error) throw before.error;
    if (!before.data) return res.status(404).json({ error: "User membership not found" });
...
router.patch("/users/:id/status", requireStoreRole("admin"), async (req: AuthRequest, res: Response) => {
  const storeId = ensureStoreId(req, res);
  if (!storeId) return;
...
router.get("/shopify/status", async (req: AuthRequest, res: Response) => {
  ...
  try {
    ...
    if (result.error && isSchemaCompatibilityError(result.error)) {
      ...
      return res.json({
        connected: Boolean(fallback.data?.connected_at || fallback.data?.is_configured),
...
router.post("/shopify/disconnect", requireStorePermission("shopify.manage"), async (req: AuthRequest, res: Response) => {
  const storeId = ensureStoreId(req, res);
  if (!storeId) return;
  ...
  try {
    ...
    const connectionUpdate = await supabase
      ...
      .eq("store_id", storeId);
    if (connectionUpdate.error && !isSchemaCompatibilityError(connectionUpdate.error)) {
...
router.post("/shopify/connect", requireStorePermission("shopify.manage"), async (req: AuthRequest, res: Response) => {
  const storeId = ensureStoreId(req, res);
  if (!storeId) return;
...
router.post("/shopify/sync/full", requireStorePermission("shopify.manage"), async (req: AuthRequest, res: Response) => {
  const storeId = ensureStoreId(req, res);
  if (!storeId) return;
  ...
  try {
    const runInsert = await supabase
      ...
      .insert({
        ...
        started_at: nowIso(),
        summary_json: { status: "running", requested_by: req.user?.id || null },
      })
      .select("id")
      .maybeSingle();
    const runId = runInsert.data?.id || null;

    const summary = await shopifySyncService.syncBrand(storeId, { wipeExistingData });

    if (runId) {
      const runUpdate = await supabase
        .from("shopify_sync_runs")
        .update({ finished_at: nowIso(), summary_json: { status: "succeeded", ...summary } })
        .eq("id", runId);
      if (runUpdate.error && !isSchemaCompatibilityError(runUpdate.error)) {
        logger.warn("Failed to update sync run", { error: runUpdate.error.message });
      }
    }

    return res.json({ success: true, summary });
  } catch (error: any) {
    logger.error("App shopify sync failed", { error: error?.message, storeId });
    return res.status(500).json({ error: error?.message || "Failed to run full sync" });
  }
});
...
router.get("/notifications/unread-count", async (req: AuthRequest, res: Response) => {
  ...
  try {
    ...
    logger.error("App notifications unread-count failed", { error: error?.message });
...
router.patch("/notifications/:id/read", async (req: AuthRequest, res: Response) => {
  ...
  try {
    ...
    logger.error("App notification read failed", { error: error?.message, notificationId });
